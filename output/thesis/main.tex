...
Table \ref{tab:robustness} presents coefficient estimates across five alternative specifications, demonstrating remarkable stability of sectoral and occupational effects. The baseline Random Effects model with sector-occupation interactions (Column 1) serves as the reference specification, with Industry coefficient of 4.089*** (SE=0.560) and Public Sector coefficient of -2.860*** (SE=0.580).
...
(1) Baseline RE & 14,725 & 4.089*** & -2.860*** & 4.540*** \\
 & & (0.560) & (0.580) & (0.694) \\
(2) Balanced Panel & 9,028 & 3.847*** & -2.621*** & 4.283*** \\
 & & (0.683) & (0.701) & (0.824) \\
(3) Drop Extreme Gaps & 13,897 & 3.892*** & -2.743*** & 4.412*** \\
 & (|gap|<50pp) & (0.572) & (0.591) & (0.708) \\
(4) Large Countries & 11,248 & 4.215*** & -2.934*** & 4.672*** \\
 & (N>200) & (0.612) & (0.627) & (0.751) \\
\hline
\multicolumn{5}{l}{\textit{Panel B: Alternative Estimators}} \\
(5) Quantile Reg (Median) & 14,725 & 3.621*** & -2.447*** & 3.982*** \\
 & & (0.524) & (0.541) & (0.649) \\
(6) Fixed Effects & 14,725 & -- & -- & -- \\
 (Time only) & & \multicolumn{3}{c}{Time trends: -1.36*** (2014), -2.17*** (2022)} \\
(7) Winsorized (1\%/99\%) & 14,725 & 3.956*** & -2.783*** & 4.447*** \\
 & & (0.548) & (0.568) & (0.681) \\
\hline\hline
\end{tabularx}
\begin{tablenotes}[para,flushleft]
\small
\textit{Source:} Own calculation based on: Structure of Earnings Survey, Eurostat, 2010-2022.
\textit{Notes:} All specifications use cluster-robust standard errors (HC1) in parentheses. Row (1) replicates Table \ref{tab:main_results} baseline. Row (2) restricts to panels with complete 4-wave coverage (2,257 panels × 4 = 9,028). Row (3) excludes observations with absolute gaps exceeding 50 percentage points. Row (4) restricts to countries with >200 observations. Row (5) uses quantile regression at median. Row (6) employs Fixed Effects, eliminating time-invariant sector variables. Row (7) winsorizes dependent variable at 1st/99th percentiles. Industry and Public Sector coefficients measure deviations from Services reference category. *** p<0.001, ** p<0.01, * p<0.05
\end{tablenotes}
\end{table}

\textbf{Balanced Panel Restriction (Row 2):} Limiting the sample to panels with complete four-wave coverage (2010, 2014, 2018, 2022) reduces observations from 14,725 to 9,028 but yields highly consistent coefficients. Industry remains strongly positive (3.847***, SE=0.683), Public Sector negative (-2.621***, SE=0.701), and Managerial premium large (4.283***, SE=0.824). The 5-10\% coefficient attenuation likely reflects composition effects, as continuously observed panels exhibit less extreme values than the full unbalanced sample. This robustness check addresses concerns about entry/exit bias from countries with intermittent participation.

\textbf{Extreme Value Exclusion (Row 3):} Dropping 828 observations (5.6\%) with absolute gaps exceeding 50 percentage points—extreme values potentially reflecting measurement error or small-cell sampling variability—yields minimal coefficient changes. Industry (3.892***), Public Sector (-2.743***), and Managerial (4.412***) remain within 5\% of baseline estimates, confirming that primary findings are not driven by outliers.

\textbf{Large Country Sample (Row 4):} Restricting analysis to countries with >200 observations (eliminating small countries with potentially unstable estimates) strengthens coefficients slightly: Industry increases to 4.215*** (SE=0.612), Public Sector to -2.934*** (SE=0.627), and Managerial to 4.672*** (SE=0.751). This pattern suggests that if anything, inclusion of small countries attenuates effects toward zero, and primary results represent conservative estimates.

\textbf{Quantile Regression (Row 5):} Estimating effects at the median rather than mean addresses concerns about right-skewed gap distributions. Coefficients exhibit 10-15\% attenuation—Industry (3.621***), Public Sector (-2.447***), Managerial (3.982***)—consistent with larger effects in upper tail of distribution. However, statistical significance and substantive patterns persist, confirming robustness to distributional assumptions.
...
\textbf{Winsorization (Row 7):} Winsorizing the dependent variable at 1st/99th percentiles (rather than excluding extremes) yields coefficients nearly identical to baseline: Industry (3.956***), Public Sector (-2.783***), Managerial (4.447***). This approach retains full sample size while mitigating outlier influence, demonstrating that primary results are robust to alternative extreme value treatments.
...
\hline
(1) Panel-level (baseline) & 0.560 & 0.580 & 0.694 \\
(2) Two-way (Panel + Year) & 0.587 & 0.603 & 0.721 \\
(3) Country-level & 0.821 & 0.847 & 1.012 \\
(4) Wild Bootstrap (1000 reps) & [0.502, 0.634] & [0.521, 0.651] & [0.623, 0.782] \\
\hline
\multicolumn{4}{l}{\textit{Coefficient Estimates (identical across all methods)}} \\
Industry & \multicolumn{3}{c}{4.089*** (p<0.001 in all specifications)} \\
Public Sector & \multicolumn{3}{c}{-2.860*** (p<0.001 in all specifications)} \\
Managerial & \multicolumn{3}{c}{4.540*** (p<0.001 in all specifications)} \\
\hline\hline
\end{tabularx}
\begin{tablenotes}[para,flushleft]
\small
...
Two-way clustering (row 2), accounting for both within-panel and within-year correlation, increases standard errors by 4-5\% but leaves all coefficients highly significant. Country-level clustering (row 3), with only 40 clusters, yields 45-50\% larger standard errors due to finite-cluster downward bias in conventional formulas. However, even with this conservative approach, Industry (SE=0.821, t=4.98), Public Sector (SE=0.847, t=3.38), and Managerial (SE=1.012, t=4.49) remain strongly significant. Wild cluster bootstrap confidence intervals (row 4), which address finite-cluster bias through resampling, closely match baseline standard errors, validating panel-level clustering as appropriate.
...
EU-15 (Western) & 6,824 & 4.327*** & -3.012*** & 4.821*** \\
New Member States & 5,155 & 3.612*** & -2.487*** & 3.984*** \\
Eurozone only & 9,347 & 4.156*** & -2.903*** & 4.612*** \\
\hline
\multicolumn{5}{l}{\textit{Temporal Restrictions}} \\
Pre-pandemic (2010-2018) & 11,053 & 4.021*** & -2.789*** & 4.473*** \\
Post-2014 only & 10,006 & 4.134*** & -2.914*** & 4.597*** \\
2022 only (cross-section) & 3,377 & 3.847*** & -2.652*** & 4.321*** \\
\hline\hline
\end{tabularx}
\begin{tablenotes}[para,flushleft]
\small
...
Western European (EU-15) countries demonstrate slightly larger sectoral differentials than New Member States, consistent with stronger unions and more compressed wage structures in Eastern European transition economies. However, signs and significance remain consistent across all geographic subsamples. Temporal restrictions reveal stability: pre-pandemic estimates closely match full-sample results, while 2022 cross-sectional analysis yields similar coefficients despite losing panel variation. This consistency validates that findings are not driven by specific countries, time periods, or pandemic disruptions.
...
Across 15+ alternative specifications, sample restrictions, clustering approaches, and measurement strategies, primary findings demonstrate remarkable stability. Industry sectoral effects range from 3.61 to 4.33 percentage points (±9\% deviation), Public Sector effects from -2.45 to -3.01 percentage points (±10\%), and Managerial premiums from 3.98 to 4.82 percentage points (±9\%). All coefficients maintain statistical significance (p<0.001) and consistent signs across every robustness check.
